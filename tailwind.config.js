...
module.exports = {
  ...
  theme: {
    extend: {
      ...
      ringColor: { DEFAULT: "rgb(var(--focus-ring) / <alpha-value>)" },
      borderRadius: { xl2: "1rem" },
      boxShadow: { panel: "0 4px 24px rgba(0,0,0,0.3)" },
      backgroundImage: { navyGradient: "linear-gradient(to bottom, #0e1b2a, #0b1522)" },
    },
  },
  plugins: [
    plugin(function({ addUtilities }) {
      addUtilities({
        ...
        // if you ever target <svg> directly (not via our Icon sanitizer), this helps:
        '.crisp-edges': { shapeRendering: 'crispEdges' },
      });
    }),
  ],
};